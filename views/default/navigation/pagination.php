...
} else if (elgg_is_xhr() && !empty($_SERVER['HTTP_REFERER'])) {
...
} else if (($start_page + 5) == ($total_pages - 1)) {
	$pages[$total_pages - 1] = [];
}
...
foreach ($pages as $page_num => $page) {
	if ($page_num == $current_page) {
		...
		} else if (!empty($page['prev'])) {
			$class[] = 'pagination-previous';
		} else if (!empty($page['next'])) {
...
$pagination = elgg_format_element('ul', ['class' => 'elgg-pagination pagination-list'], $list);
